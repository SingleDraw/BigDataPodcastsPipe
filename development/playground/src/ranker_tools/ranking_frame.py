...
from src.ranker_tools.fuzzy_clusterer import FuzzyClusterer
...
class RankingFrame(pd.DataFrame):
    ...
    def _validate_fields(
        ...
        """
        print("Validating fields in RankingFrame...")
    ...
    def filter_top_ranked(
        ...
        if top > min_group_size:
            ...
            # )
            top = min_group_size
            print(f"Adjusting top N to {top} based on the smallest group size ({min_group_size}).")
        else:
            print(f"Filtering top {top} ranked podcasts per group wher the smallest group size is {min_group_size}.")
