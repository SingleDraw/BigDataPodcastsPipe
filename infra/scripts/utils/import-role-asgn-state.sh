...
if [[ -n "$ROLE_ASSIGNMENT" ]]; then
  echo "Checking if $FULL_RESOURCE_PATH is already managed..."
  if terraform state list | grep -q "$FULL_RESOURCE_PATH"; then
    echo "$FULL_RESOURCE_PATH is already managed by Terraform. Skipping import."
  else
    echo "Importing $FULL_RESOURCE_PATH..."
    terraform import "$FULL_RESOURCE_PATH" "$ROLE_ASSIGNMENT"
    echo "Import of $FULL_RESOURCE_PATH successful."
  fi
else
  echo "Role assignment $ROLE_RESOURCE_NAME at $ROLE_SCOPE does not exist yet. Skipping import attempt."
fi