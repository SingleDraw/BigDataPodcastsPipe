...
class PCaster:
    ...
    def __init__(
        ...
        self.podcasts_blob_path = f"{sink_dir}/date={self.today}/top_podcasts.parquet"
        self.results_blob_path = f"{sink_dir}/date={self.today}/scrape_job_status.parquet"
    ...
    def _check_existing_blobs(
        ...
        if self.overwrite:
            ...
            self.storage_client.blob_exists(
                self.container_name, 
                self.podcasts_blob_path
            ) and 
            self.storage_client.blob_exists(
                self.container_name, 
                self.results_blob_path
            )
        ):
            logger.info("Blobs already exist. Exiting to avoid overwriting.")
            self.valid = False
            return
    ...
    def _set_client(
        ...
        self.storage_client = self.storage_client_constructor(
            ...
            azure_storage_account = self.storage_credentials.get("azure_storage_account", ""),
            azure_storage_key = self.storage_credentials.get("azure_storage_key", "")
        )
    ...
    def upload_results(
        ...
        self.runner(
            ...
            container_name=self.container_name,
            blob_path=self.podcasts_blob_path,
            df=self.scraped["top_podcasts"]
        )
        self.runner(
            self.storage_client.upload_df_as_parquet,
            container_name=self.container_name,
            blob_path=self.results_blob_path,
